# Review: Canonical Task Command Migration 005
...
Issue 04 routes Telegram and dashboard Task capture through one authenticated application command.
This migration provides its atomic PostgreSQL boundary: canonical Task states, aggregate version,
tenant-owned command receipts, payload-conflict detection, and replay of the stored result.

Existing rows are preserved with `done` mapped to `completed` and `deferred` mapped to `pending`.
The legacy `created_date` remains as audit compatibility data, but current-day reads now use
`due_date`; a Task with no Planning Day therefore remains in Inbox.
...
- [x] Existing `done` and `deferred` rows have safe canonical mappings.
- [x] Browser roles lose direct Task INSERT access.
- [x] Command receipts are backend-only and unique per participant/key.
- [x] The advisory transaction lock prevents concurrent duplicate Task creation.
- [x] Replay returns the stored result and conflicting payloads fail before mutation.
- [x] Source Session ownership is checked explicitly.
- [x] The service-role function exposes no client-selected identity through HTTP/Telegram adapters.
