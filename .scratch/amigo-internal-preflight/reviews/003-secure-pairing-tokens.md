# Review: Secure Pairing Tokens Migration 003
...
This proposed migration makes Pairing-token access backend-only, adds durable issuance auditing,
atomically invalidates replacement tokens, enforces five token generations per Dashboard Account
per 15 minutes, and atomically links a Dashboard Account to a Telegram Profile without allowing
either identity to be reassigned.

The application code expects the two RPCs below. Do not deploy that code before this migration.
...
- [x] Confirm five generations per 15 minutes is an acceptable Internal Preflight limit.
- [x] Confirm replacement should invalidate every older unconsumed token for the Dashboard Account.
- [x] Confirm a conflicting identity leaves the token unconsumed so it can still be used with its
  intended Telegram Profile.
- [x] Confirm expired tokens may be deleted after 24 hours during the next issuance for that
  Dashboard Account.
- [x] Confirm only `service_role` should execute either RPC or access Pairing-token rows.

## Decision

Approved by the project owner on 2026-08-30. The reviewed SQL was copied to
`migrations/003_secure_pairing_tokens.sql` without semantic changes.
