...
| Security assurance is incomplete | CI applies every checked-in migration through 014 and runs two-participant RLS regression assertions, the Activation lock-order probe, and the schema-chain guard, but dependency audits and secret scanning are still absent. | **Partially resolved locally** | Add dependency and secret scans. | Before launch |
...
| Exact-version startup is proven only locally | README includes migrations 001–014, and CI applies the whole chain, its assertions, and `scripts/check_schema_chain.py`. The gate has never run against a real Supabase/PostgREST deployment, so the RPC response shape the gate reads is unverified. | **Medium** | Boot the staged application against staging and confirm `get_app_schema_version` satisfies the exact-version gate. | Before deploying this worktree |
